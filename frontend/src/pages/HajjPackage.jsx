...
const HajjPackage = () => {
  ...
  const fetchPackages = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/products/category/holy-hajj')
  ...
  return (
    <div className="min-h-screen py-8 bg-gray-900">
      <div className="container px-4 mx-auto">
        <h1 className="mb-8 text-4xl font-bold text-center">Hajj Packages</h1>
        ...
        {packages.length === 0 && (
          <p className="mt-8 text-center text-gray-400">No Hajj packages available yet</p>
        )}
      </div>
    </div>
  )
